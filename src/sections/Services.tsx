      ...
      <div className="container-premium">
        ...
        <div className="mt-12 grid gap-5 md:grid-cols-2">
          {services.map((service) => {
            ...
            return (
              <AnimatedSection key={service.title} className="group rounded-[28px] border border-white/10 bg-white/[0.045] p-6 transition duration-300 hover:-translate-y-1 hover:border-brand-bright/50 hover:bg-white/[0.075] sm:p-8">
                ...
                <p className="mt-4 text-base leading-8 text-metal-300">{service.description}</p>
                <div className="mt-6 flex flex-wrap gap-2">
                  {service.bullets.map((bullet) => (
                    <span key={bullet} className="rounded-full border border-white/10 bg-graphite-950/70 px-3 py-1.5 text-xs font-medium text-metal-100">
                      {bullet}
                    </span>
                  ))}
                </div>
        ...
        <AnimatedSection className="mt-12 rounded-[28px] border border-white/10 bg-gradient-to-r from-brand-deep/50 via-white/[0.05] to-graphite-800 p-6 sm:p-8 lg:flex lg:items-center lg:justify-between">
          <div>
            <h3 className="text-2xl font-semibold text-white">Tem um projeto técnico para avaliar?</h3>
            <p className="mt-3 max-w-2xl text-sm leading-7 text-metal-300">
              Envie desenhos, aplicação, material desejado, volume estimado ou uma descrição do problema. A equipe retorna com o caminho mais adequado.
            </p>
          </div>
